...
@injectable()
export class BgpVpnv4PeerIngestor implements IIngestor {
    readonly blockType = "display_bgp_vpnv4_peer_block";
    ...
    async ingest(block: ParserBlock, context: IngestionContext): Promise<void> {
        ...
        if (peersToUpsert.length > 0) {
            ...
            console.log(`[BgpVpnv4PeerIngestor] Upserted ${peersToUpsert.length} VPNv4 BGP peers.`);
        }
    }
}

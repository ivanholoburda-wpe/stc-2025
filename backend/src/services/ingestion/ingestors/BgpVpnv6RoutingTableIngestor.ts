...
@injectable()
export class BgpVpnv6RoutingTableIngestor implements IIngestor {
    readonly blockType = "display_bgp_vpnv6_routing_table_block";
    readonly priority = 162;
    ...
    async ingest(block: ParserBlock, context: IngestionContext): Promise<void> {
        ...
        if (routesToUpsert.length > 0) {
            ...
            console.log(`[BgpVpnv6RoutingTableIngestor] Upserted ${routesToUpsert.length} VPNv6 routes.`);
        }
    }
}
